...
監査・内部統制用途では、結論だけでなく、判断過程と証跡が重要である。各エージェントの行動は、以下のような形式で残す。
...
```json
{
  ...
  "actor": "requester_A",
  "role": "申請者",
  "action": "購買申請を2件に分割して登録",
  "amount": 900000,
  "vendor": "Vendor-014",
  "related_case_id": "PO-CASE-019",
  "reason": "部長承認では納期に間に合わないと判断",
  "referenced_policy": "購買規程 第X条",
  "control_result": "課長承認で通過",
  "risk_flags": ["金額分割の疑い", "承認閾値回避"]
}
```
...
|---|---|---|---|
| 申請者 | 必要な物品・サービスを期限内に調達する | 申請、金額分割、緊急申請、事前相談 | 納期・予算圧力による統制回避 |
| 承認者 | 部門業務を止めずにリスクを抑える | 承認、差戻し、代理承認、コメント依頼 | 形式的承認、確認不足、代理承認の妥当性 |
| 購買担当 | 規程に沿って発注しつつ納期を守る | 見積確認、発注、例外処理、取引先選定 | 比較見積省略、緊急発注の濫用 |
| 経理担当 | 請求・支払を正確かつ期限内に処理する | 請求書照合、支払承認、差戻し | 検収不足、支払遅延回避による手作業処理 |
