...
stateDiagram-v2
    [*] --> draft
    draft --> pending_approval: create_purchase_request
    draft --> emergency_ordered: select_request_route(route_type=emergency)
...
| payment_blocked | 3点照合差異や検収不足により支払停止 | 支払遅延、手作業処理への迂回 |
...
`select_request_route` の `route_type` は `normal` / `emergency` に限定する。`route_type=normal` の場合は通常承認ルートへ進む。`route_type=emergency` の場合は `emergency_ordered` へ進み、事後承認を要求する。
