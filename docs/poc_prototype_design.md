...
inputs:
  ...
  purchase_cases: data/synthetic/purchase_cases.csv
evaluation_inputs:
...
behavioral_effects:
  requester:
    action_bias:
      create_purchase_request: 0.0
      select_request_route_emergency: 0.2
      consult_manager: -0.1
      postpone_request: -0.3
  approver:
    action_bias:
      approve: 0.2
      request_revision: -0.2
      request_additional_evidence: -0.1
  purchasing:
    action_bias:
      request_quotes: -0.1
      approve_quote_exception: 0.2
expected_risks:
  - P2P-R-001
  - P2P-R-002
```
...
イベントログは、すべての分析の基礎になる。1イベント1行のJSONLで保存する。
...
```json
{
  ...
  "case_id": "PO-CASE-001",
  "parent_case_id": "PO-CASE-ROOT-001",
  ...
  "state_before": "draft",
  "state_after": "pending_manager_approval",
  ...
  "risk_flags": ["possible_split_order"],
...
- `purchase_cases.csv` には `seeded_deficiency_id` を含めない
...
stateDiagram-v2
    [*] --> Draft
    Draft --> PendingApproval: create_purchase_request
    Draft --> EmergencyOrdered: select_emergency_route
    PendingApproval --> EmergencyOrdered: emergency_override
    PendingApproval --> Rejected: reject
    PendingApproval --> Returned: request_revision
    Returned --> Draft: revise
    PendingApproval --> Approved: approve
    Approved --> PurchaseOrderIssued: issue_purchase_order
    EmergencyOrdered --> PostApprovalPending: require_post_approval
    PostApprovalPending --> PurchaseOrderIssued: approve_post_facto
    PostApprovalPending --> ExceptionEscalated: post_approval_overdue
    PurchaseOrderIssued --> GoodsReceived: receive_goods
    PurchaseOrderIssued --> InvoiceReceived: receive_invoice_before_goods
    GoodsReceived --> InvoiceReceived: receive_invoice
    InvoiceReceived --> Matched: three_way_match_passed
    InvoiceReceived --> MatchException: three_way_match_failed
    MatchException --> Matched: resolve_exception
    MatchException --> PaymentBlocked: unresolved_exception
    Matched --> PaymentApproved: approve_payment
    PaymentApproved --> Paid: execute_payment
    Paid --> [*]
```
...
| EmergencyOrdered | 緊急発注 | 事前統制の省略、例外の常態化 |
| PostApprovalPending | 事後承認待ち | 48時間超過、形式的承認 |
| MatchException | 3点照合差異 | 過大請求、重複請求、検収不足 |
| ExceptionEscalated | 事後承認期限超過などにより上位者確認が必要 | 例外処理の滞留、形式的承認 |
| PaymentBlocked | 3点照合差異や検収不足により支払停止 | 支払遅延、手作業処理への迂回 |
| AgentActionFailed | LLMまたはAction検証失敗により処理不能 | 再現性不足、ActionDefinition不足 |
| Deadlocked | 実行可能なActionがなく、案件が進まない | 状態遷移設計漏れ |
| MaxStepsReached | 最大Step数に到達し、強制終了 | ループ、例外処理滞留 |

### 8.1 Simulation Step の定義
...
9. Event Storeに、Action、状態遷移、統制判定結果、LLM call_id、risk_flagsを保存する
...
|---|---|---|---|
| create_purchase_request | 購買申請を1件作成する | amount, vendor_id, project_id, item_description, business_reason, requested_delivery_date | 分割かどうかはDetectorが後から判定する |
| select_request_route | 通常・緊急などの申請ルートを選ぶ | route_type, reason | `route_type` に normal / emergency を持たせる |
...
| manual_payment_override | 手作業で支払処理する | override_reason |
...
| side_effects | 案件、関連案件、イベント、risk_flagsへの影響 |
...
1. 同一 `requester_id`
2. 同一 `vendor_id`
3. 同一または類似 `project_id`
4. 申請日が `window_days` 以内
5. 個別金額は部長承認閾値未満
6. 合算金額は部長承認閾値以上
7. 個別申請が同一承認階層で承認されている
...
| run_name | scenario | variant | seed |
|---|---|---|---:|
| baseline_s002_seed1 | S-002 | baseline | 101 |
| variant_a_s002_seed1 | S-002 | variant_a | 101 |
| variant_b_s002_seed1 | S-002 | variant_b | 101 |
| baseline_s002_seed2 | S-002 | baseline | 102 |
| variant_a_s002_seed2 | S-002 | variant_a | 102 |
...
| イベントログ | 案件ごとの状態遷移を追う | タイムライン、 actor、action、control_result、risk_flags |
...
| D-008 | 架空・関連当事者取引先 | 次フェーズ | 2 | 取引先属性と関連当事者データの設計が必要 |

D-008は、初回PoCでは設計候補として残し、実装対象からは外す。
